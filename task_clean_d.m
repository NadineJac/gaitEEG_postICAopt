function task_clean_d(sii, PATH, cfg)
...
% In contrast to recommendations for repetitive clustering, only cluster dipolar (residual variance
% <15%) IC with sources inside the head (similar to Wagner, Ferris, etc)
...
% [file] clustered STUDY in PATHOUT named [file_out, 'clean_d'] and
...
% difference to pipeline C: only keep dipoles located inside the head and
% with residual variance lower than 15 %

df{end+1} = {'inbrain','on','dipselect',0.15};
%% create study
...
[STUDY, ALLEEG, EEG] = bemobil_repeated_clustering_and_evaluation(STUDY, ALLEEG, EEG,...
    ...
    0, 1, STUDY.filepath,[cfg.study_name, '_clean_d'],...
